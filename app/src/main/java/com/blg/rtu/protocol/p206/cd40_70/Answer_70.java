...
public class Answer_70  extends ProtocolSupport{

	private static String tag = Answer_70.class.getName() ;
	/**
	 * 解析上行数据 
	 * @param rtuId RTU ID
	 * @param b 上行数据
	...
	public RtuData parse(String rtuId, byte[] b, ControlProtocol cp, String dataCode) throws Exception {
		...
		Log.i(tag, "分析<RTU查询净积>应答: RTU ID=" + rtuId + " 数据：" + d.getSubData().toString());
		return d;
	}
	private void doParse(byte[] b, int index, RtuData d, ControlProtocol cp) throws Exception {
		Data_70 dd = new Data_70() ;
		d.setSubData(dd) ;
