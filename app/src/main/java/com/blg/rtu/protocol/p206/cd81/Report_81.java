package com.blg.rtu.protocol.p206.cd81;
...
public class Report_81  extends ProtocolSupport{

	private static String tag = Report_81.class.getName() ;
	...
	public RtuData parse(String rtuId, byte[] b, ControlProtocol cp, String dataCode) throws Exception {
		...
		Log.i(tag, "分析<随机自报报警数据>应答: RTU ID=" + rtuId + " 数据：" + d.getSubData().toString());
		return d;
	}
...
//		}
	...
	protected void doParseRain(byte[] b, int index, RtuData d, ControlProtocol ca) throws Exception {
		DataMap_81 dataMap = new DataMap_81() ;
...
//	protected void doParseStaticRain(byte[] b, int index, Data d, ControlAgree ca) throws Exception {
//		DataMap_81 dataMap = new DataMap_81() ;
//		d.setSubData(dataMap) ;
//		
//		index = this.parseStatus(b, index, dataMap) ;
...
//	}
	...
	protected void doParseWaterLevel(byte[] b, int index, RtuData d, ControlProtocol ca) throws Exception {
		...
		for(int i = 0 ; i < total ; i++){
			plus = true ;
			Data_waterLevel_81 dd = new Data_waterLevel_81() ;
...
//		this.parseTime(d, b, index) ;
	...
	protected void doParseWaterAmount(byte[] b, int index, RtuData d, ControlProtocol ca) throws Exception {
		...
		for(int i = 0 ; i < total ; i++){
			plus = true ;
			Data_waterAmount_81 dd = new Data_waterAmount_81() ;
			dataMap.setValue(i + 1, dd) ;
...
//		this.parseTime(d, b, index) ;
	...
	protected int parseStatus(byte[] b, int index, DataMap_81 dataMap) throws Exception {
		AlarmStatusProtocol_206 asAgree = new AlarmStatusProtocol_206() ;
		asAgree.parseAlarm(b, index, dataMap) ;
