...
public class Answer_15 extends ProtocolSupport{

	private static String tag = Answer_15.class.getName() ;
	/**
	 * 解析上行数据 
	 * @param rtuId RTU ID
	 * @param b 上行数据
	...
	public RtuData parse(String rtuId, byte[] b, ControlProtocol cp, String dataCode) throws Exception {
		...
		Log.i(tag, "分析<RTU设置遥测终端本次充值量>应答: RTU ID=" + rtuId + " 数据：" + d.getSubData().toString());
		return d;
	}
	private void doParse(byte[] b, int index, RtuData d, ControlProtocol cp) throws Exception {
		Data_15 dd = new Data_15() ;
		d.setSubData(dd) ;
